...
-- Oracle package 'ng_rep_other' declaration, please edit to match PostgreSQL syntax.
CREATE TYPE ng_rep_other.rec AS (
kod_price_zone bigint
              , zone_name     varchar(30)
              , gtp_gp_name   varchar(200)
              , gtp_gp_kod_region  bigint
              , gtp_gp_region_name varchar(200)
              , kod_gtp         bigint
              , kod_gtp_letter  varchar(250)
              , gtp_name        varchar(200)
              , date_admission  timestamp(0)
              , ym bigint
              , buy_rsv_value   bigint
              , buy_rsv_money   bigint
              , sell_rsv_value  bigint
              , sell_rsv_money  bigint
              , fine_pwr_money  bigint
              , buy_pwr_value   bigint
              , buy_pwr_money   bigint
              , buy_pwr_frsvr_value  bigint
              , buy_pwr_frsvr_money  bigint
              , fine_pwr_not_ready_vr_money bigint
              , penalty_pwr_dvr_money       bigint
              , penalty_pwr_over_money    bigint
              , fine_pwr_not_ready_money  bigint
              , penalty_pwr_kom_money     bigint
              , buy_pwr_over_value      bigint
              , buy_pwr_trans_money    bigint
              , buy_pwr_kom_value      bigint
              , buy_pwr_kom_money      bigint
              , fine_pwr_kommod_money  bigint
              , buy_pwr_kommod_value   bigint
              , buy_pwr_kommod_money   bigint
              , buy_pwr_con_value     bigint
              , buy_pwr_con_money     bigint
              , fine_pwr_con_money    bigint
              , buy_pwr_dpm_value     bigint
              , buy_pwr_dpm_money     bigint
              , buy_pwr_dpmga_value   bigint
              , buy_pwr_dpmga_money   bigint
              , buy_br_value          bigint
              , buy_br_money          bigint
              , sell_br_value         bigint
              , sell_br_money         bigint
              , rd_value_e            bigint
              , rd_money_e            bigint
              , rd_value_p            bigint
              , rd_money_p            bigint
              , upz_sell_energy_value_pe bigint
              , upz_sell_energy_value    bigint
              , upz_sell_energy_money    bigint
              , upz_sell_power_value     bigint
              , upz_sell_power_money     bigint
              , fact_rd_dv_value_e  bigint
              , fact_rd_dv_money_e  bigint
              , rd_dv_value_p  bigint
              , rd_dv_money_p  bigint
              , likom_buy_value  bigint
              , likom_buy_money  bigint
              , buy_pwr_kommod_rd_value  bigint
              , buy_pwr_kommod_rd_money  bigint
              , upr_shrt_money  bigint
             
);

CREATE TYPE ng_rep_other.tblats AS (tblats ng_rep_other.rec[]);

CREATE TYPE ng_rep_other.phonemail AS (
kodp bigint
              , phone_all   varchar(500)
              , mail_all    varchar(500)
              
);

CREATE TYPE ng_rep_other.tblphonemail AS (tblphonemail ng_rep_other.PhoneMail[]);
...
CREATE OR REPLACE FUNCTION ng_rep_other_get_tbl_ats_data (p_ym_beg bigint, p_ym_end bigint, p_coeff bigint default -1) RETURNS SETOF TBLATS AS $body$
DECLARE

     cnt_kod_adr_m bigint;  -- кол-во выбранных субъектов РФ
  curr RECORD;
...
BEGIN
    ...
    FOR curr IN
        (
           SELECT   CASE WHEN coalesce(g.kod_price_zone,0) in (63,64) THEN 62 ELSE coalesce(g.kod_price_zone,0) END  as kod_price_zone  -- код ценовой зоны (Хабаровск и Дальний восток сейчас находятся во 2 ЦЗ)
                  , max(coalesce((CASE WHEN g.kod_price_zone in (63,64) THEN 'Вторая ценовая зона' ELSE i.name END),'Пустая зона')) as zone_name   -- ценовая зона (наименование)
                  , coalesce(g_gp.name,'-') as gtp_gp_name   -- наименование ГТП ГП (по '-' формируем данные ГТП ГП для второго листа отчета)
              ...
              , max(coalesce(g.name,'-')) as gtp_name
              , g.date_admission as date_admission
              , r.ym

              , SUM(CASE WHEN k.name = 'buy_rsv' AND r.value_type = 'E' THEN coalesce(value,0) ELSE 0 END) as buy_rsv_value   -- kod_ats_data = 1
              , SUM(CASE WHEN k.name = 'buy_rsv' AND r.value_type = 'E' THEN coalesce(money,0) ELSE 0 END) as buy_rsv_money
              , SUM(CASE WHEN k.name = 'sell_rsv' AND r.value_type = 'E' THEN coalesce(value,0) ELSE 0 END) * p_coeff as sell_rsv_value   -- kod_ats_data = 2
              , SUM(CASE WHEN k.name = 'sell_rsv' AND r.value_type = 'E' THEN coalesce(money,0) ELSE 0 END) * p_coeff as sell_rsv_money
              , SUM(CASE WHEN k.name = 'fine_pwr' AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END) * -1 as fine_pwr_money  -- kod_ats_data = 3
              , SUM(CASE WHEN k.name = 'buy_pwr'  AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_value   -- kod_ats_data = 4
              , SUM(CASE WHEN k.name = 'buy_pwr'  AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_money
              , SUM(CASE WHEN k.name = 'buy_pwr_frsvr' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_frsvr_value   -- kod_ats_data = 5
              , SUM(CASE WHEN k.name = 'buy_pwr_frsvr' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_frsvr_money

              , SUM(CASE WHEN k.name = 'fine_pwr_not_ready_vr' AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END) * -1  as fine_pwr_not_ready_vr_money  -- kod_ats_data = 6
              , SUM(CASE WHEN k.name in ('buy_pwr_frsvr', 'fine_pwr_not_ready_vr') AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END) * -1  as penalty_pwr_dvr_money  -- стоимость штрафов по генераторам для п. 4.2.4
              , SUM(CASE WHEN k.name = 'penalty_pwr_over' AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END) as penalty_pwr_over_money   -- kod_ats_data = 8
              , SUM(CASE WHEN k.name = 'fine_pwr_not_ready' AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END) as fine_pwr_not_ready_money   -- kod_ats_data = 21
              , SUM(CASE WHEN k.name in ('penalty_pwr_over', 'fine_pwr_not_ready') AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END) * -1 as penalty_pwr_kom_money   -- стоимость штрафов по генераторам для п. 4.2.5
              , SUM(CASE WHEN k.name = 'buy_pwr_over' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_over_value   -- kod_ats_data = 7
              , SUM(CASE WHEN k.name = 'buy_pwr_trans' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_trans_money   -- kod_ats_data = 9
              , SUM(CASE WHEN k.name in ('buy_pwr_over', 'buy_pwr_trans') AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_kom_value   -- kod_ats_data = 7+9 для п.4.2.5 for the value
              , SUM(CASE WHEN k.name in ('buy_pwr_over', 'buy_pwr_trans') AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_kom_money   -- kod_ats_data = 7+9 для п.4.2.5 for money
              , SUM(CASE WHEN k.name = 'fine_pwr_kommod' AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END) * -1 as fine_pwr_kommod_money  -- kod_ats_data = 12
              , SUM(CASE WHEN k.name = 'buy_pwr_kommod' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_kommod_value   -- kod_ats_data = 13
              , SUM(CASE WHEN k.name = 'buy_pwr_kommod' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_kommod_money

              , SUM(CASE WHEN k.name = 'buy_pwr_con' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_con_value   -- kod_ats_data = 14
              , SUM(CASE WHEN k.name = 'buy_pwr_con' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_con_money
              , SUM(CASE WHEN k.name = 'fine_pwr_con' AND r.value_type = 'P' THEN coalesce(penalty,0) ELSE 0 END)  * -1  as fine_pwr_con_money   -- kod_ats_data = 15
              , SUM(CASE WHEN k.name = 'buy_pwr_dpm' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_dpm_value   -- kod_ats_data = 16
              , SUM(CASE WHEN k.name = 'buy_pwr_dpm' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_dpm_money
              , SUM(CASE WHEN k.name = 'buy_pwr_dpmga' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_dpmga_value   -- kod_ats_data = 17
              , SUM(CASE WHEN k.name = 'buy_pwr_dpmga' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_dpmga_money
              , SUM(CASE WHEN k.name = 'buy_br' AND r.value_type = 'E' THEN coalesce(value,0) ELSE 0 END) as buy_br_value   -- kod_ats_data = 18
              , SUM(CASE WHEN k.name = 'buy_br' AND r.value_type = 'E' THEN coalesce(money,0) ELSE 0 END) as buy_br_money

              , SUM(CASE WHEN k.name = 'sell_br' AND r.value_type = 'E' THEN coalesce(value,0) ELSE 0 END) * p_coeff as sell_br_value   -- kod_ats_data = 19
              , SUM(CASE WHEN k.name = 'sell_br' AND r.value_type = 'E' THEN coalesce(money,0) ELSE 0 END) * p_coeff as sell_br_money

              , SUM(CASE WHEN k.name = 'rd' AND r.value_type = 'E' THEN coalesce(value,0) ELSE 0 END) as rd_value_e   -- kod_ats_data = 20
              , SUM(CASE WHEN k.name = 'rd' AND r.value_type = 'E' THEN coalesce(money,0) ELSE 0 END) as rd_money_e
              , SUM(CASE WHEN k.name = 'rd' AND r.value_type = 'P' THEN nvl(value,0) ELSE 0 END) as rd_value_p  -- kod_ats_data = 20 with value_type = 'P'
              , SUM(CASE WHEN k.name = 'rd' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as rd_money_p
              ...
              , SUM(CASE WHEN k.name = 'fact_rd_dv' AND r.value_type = 'E' THEN coalesce(value,0) ELSE 0 END) as fact_rd_dv_value_e
              , SUM(CASE WHEN k.name = 'fact_rd_dv' AND r.value_type = 'E' THEN coalesce(money,0) ELSE 0 END) as fact_rd_dv_money_e
              , SUM(CASE WHEN k.name = 'rd_dv' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as rd_dv_value_p
              , SUM(CASE WHEN k.name = 'rd_dv' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as rd_dv_money_p
              , SUM(CASE WHEN k.name = 'likom_buy' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as likom_buy_value
              , SUM(CASE WHEN k.name = 'likom_buy' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as likom_buy_money
              , SUM(CASE WHEN k.name = 'buy_pwr_kommod_rd' AND r.value_type = 'P' THEN coalesce(value,0) ELSE 0 END) as buy_pwr_kommod_rd_value
              , SUM(CASE WHEN k.name = 'buy_pwr_kommod_rd' AND r.value_type = 'P' THEN coalesce(money,0) ELSE 0 END) as buy_pwr_kommod_rd_money
              , SUM(CASE WHEN k.name = 'upr_shrt' THEN coalesce(money,0) ELSE 0 END) as upr_shrt_money
        ...
        FROM nr_ats_data r
             ...
             LEFT JOIN address.adr_m m ON COALESCE(g.kod_rek, g_gp.kod_rek) = m.kod_m
     ...
     LOOP
      ...
      END LOOP;
      RETURN;
   END;  -- end function get_tbl_ats_data
  ------------------------ ---------------------------------------------------
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION ng_rep_other_get_tbl_ats_data (p_ym_beg bigint, p_ym_end bigint, p_coeff bigint default -1) FROM PUBLIC;



CREATE OR REPLACE FUNCTION ng_rep_other_get_phone_all (p_kodp bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
 ...
 pre as ( -- берем все телефоны по каждому сотруднику
  SELECT kodp, trim(both REPLACE(e.tel || ',' || e.m_tel || ',' || e.p_tel,';',',')) tel
 ...
 , pre2 as (
  ...
  , pre3 as (
    SELECT kodp, trim(both (SELECT array_to_string(a, '') FROM regexp_matches(tel, '[^,]+', 'g') AS foo(a) LIMIT 1 OFFSET (lines.column_value - 1) )) as  tel
     FROM pre2,
       TABLE(CAST(MULTISET(WITH RECURSIVE cte AS (
SELECT LEVEL  instr(tel, ',', 1, LEVEL - 1) > 0
                          UNION ALL
SELECT LEVEL  instr(tel, ',', 1, LEVEL - 1) > 0
                         JOIN cte c ON ()

) SELECT * FROM cte) AS odciNumberList )) lines 
    )
 , pre4 as (
     SELECT kodp, tel
       FROM pre3
   WHERE (tel IS NOT NULL AND tel::text <> '') 
      group by kodp, tel
        )
  SELECT  -- listagg(trim(tel), ', ' || CHR(13) || CHR(10)) within group(order by kodp) as tel_all
         string_agg(trim(both tel), ', ' order by kodp) as tel_all
   INTO STRICT phone_all
   FROM pre4;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION ng_rep_other_get_phone_all (p_kodp bigint) FROM PUBLIC;



CREATE OR REPLACE FUNCTION ng_rep_other_get_email_all (p_kodp bigint) RETURNS varchar AS $body$
DECLARE

   email_all varchar(500);

BEGIN

 WITH
 pre as ( -- берем все телефоны по каждому сотруднику
  SELECT kodp, trim(both REPLACE(e.e_mail,';',',')) as email
    FROM payer.kr_employee e
    WHERE e.kodp = p_kodp
    )
 , pre2 as (
  ...
  , pre3 as (
    SELECT kodp, trim(both (SELECT array_to_string(a, '') FROM regexp_matches(email, '[^,]+', 'g') AS foo(a) LIMIT 1 OFFSET (lines.column_value - 1) )) as  email
     FROM pre2,
       TABLE(CAST(MULTISET(WITH RECURSIVE cte AS (
SELECT LEVEL  instr(email, ',', 1, LEVEL - 1) > 0
                          UNION ALL
SELECT LEVEL  instr(email, ',', 1, LEVEL - 1) > 0
                         JOIN cte c ON ()
 ...
 , pre4 as (
  ...
  SELECT  -- listagg(trim(email), ',' || CHR(13) || CHR(10)) within group(order by kodp) as email_all
         string_agg(trim(both email), ', ' order by kodp) as email_all
   INTO STRICT email_all
   FROM pre4;

  RETURN email_all;

 END;  -- end FUNCTION ng_rep_other_get_email_all(p_kodp NUMBER)
   ------------------------------------ ---------------------------------------------------
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION ng_rep_other_get_email_all (p_kodp bigint) FROM PUBLIC;



CREATE OR REPLACE PROCEDURE ng_rep_other_get_rate_per_part_year (p_year1 bigint, p_year2 bigint, p_kod_m bigint default -10000, p_kodp bigint default -10000) AS $body$
DECLARE
...
BEGIN
    ...
    SELECT s_data_id,
          ...
          from ( SELECT f.kod_fo as kod_fo,
              ...
              WHERE tip_tarif_sost = 5
                  AND (tarif_parent IS NOT NULL AND tarif_parent::text <> '')
         ...
         PIVOT(  max(part1_year1) as part1_year1, max(part2_year1) as part2_year1, max(part1_year2) as part1_year2, max(part2_year2) as part2_year2
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE ng_rep_other_get_rate_per_part_year (p_year1 bigint, p_year2 bigint, p_kod_m bigint default -10000, p_kodp bigint default -10000) FROM PUBLIC;
...
CREATE OR REPLACE PROCEDURE ng_rep_other_po_per_region (p_ym_beg bigint, p_ym_end bigint, p_is_only_active bigint default 0) AS $body$
DECLARE

        cnt_dep     bigint;   -- кол-во выбранных отделений
        cnt_kodp    bigint;   -- кол-во выбранных потребителей
        cnt_kod_adr_m bigint;  -- кол-во выбранных субъектов РФ
        cnt_kod_dog   bigint;  -- кол-во выбранных договоров
        p_no_check_price bigint; -- не нужна ли проверка: price is not null
BEGIN
      DELETE FROM rr_rep_po;
     ...
     IF (ng_rep_common.get_count_value_param('p_no_check_price') = 1)   --  p_no_check_price = 1 если нужна проверка: price is not null, иначе собираем отчет до появления фактических цен текущего месяца
       THEN Select COALESCE(val,0) into STRICT p_no_check_price  From vr_number_array Where array_id = 'p_no_check_price';
     ...
     , pre as (
              SELECT  nk_adress.kf_get_sf(CASE WHEN coalesce(ob.kodd::text, '') = '' OR ob.kodd = -1 THEN d.kod_d_dog ELSE ob.kodd END) as kod_region
                     , nk_adress.kp_get_region_name(CASE WHEN coalesce(ob.kodd::text, '') = '' OR ob.kodd = -1 THEN d.kod_d_dog ELSE ob.kodd END) as kod_region_name
                     , CASE WHEN coalesce(a.tarif,0)=0 THEN  0  ELSE kg_tarif.f_byt_cached(a.tarif) END  as pr_byt     -- категория потребителя (население или прочие: признак быт (0- не население, 1 - население))
                     , a.kod_dog
                     , a.ndog as ndog
                     , p.kodp
                     ...
                     , a.edizm
                     , (CASE WHEN coalesce(vol.voltage::text, '') = '' THEN -9 ELSE vol.voltage END) as voltage
                     , coalesce(vol.abbr,'-')  as volt_abbr
                     , a.cust    -- натуральные показатели
                     , a.price
                     , a.nach as nach        -- начислено без НДС
                     , a.nal
                     , a.nachisl
                     , coalesce(hg_dogr.get_pmax_numobj(a.kod_numobj, 2, kg.ym_first_day(a.ym), 0, a.voltage),0) AS pmax
               ...
               Where ((cnt_dep = 0) Or (cnt_dep > 0 And dp.dep in (Select val From vr_number_array Where array_id = 'p_dep')))
                   ...
                   AND (p_no_check_price = 1 OR (p_no_check_price=0 AND (a.price IS NOT NULL AND a.price::text <> '')))  -- добавлено для отчета № 32 по SD: 75956(3) 06.02.2026
                   AND coalesce(a.rym::text, '') = ''
                   AND ((p_is_only_active = 0  AND dp.pr_active in (0,2))
                         OR (p_is_only_active = 1 AND p_date_beg BETWEEN d.dat_dog AND COALESCE(d.dat_fin, clock_timestamp() + interval '5000 days')
                             OR p_date_end BETWEEN d.dat_dog AND COALESCE(d.dat_fin, clock_timestamp() + interval '5000 days')))   -- активные договоры на дату отчета 23.04.2025
...
;
   ...
   END;  -- end procedure po_per_region
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE ng_rep_other_po_per_region (p_ym_beg bigint, p_ym_end bigint, p_is_only_active bigint default 0) FROM PUBLIC;



CREATE OR REPLACE PROCEDURE ng_rep_other_po_per_region_add_regl (p_ym_beg bigint, p_ym_end bigint, p_kod_typevariant bigint default 1, p_add_rule_opt bigint default 0) AS $body$
DECLARE
...
BEGIN
     ...
     , pre as (
              SELECT  nk_adress.kf_get_sf(CASE WHEN coalesce(ob.kodd::text, '') = '' OR ob.kodd = -1 THEN d.kod_d_dog ELSE ob.kodd END) as kod_region
                     ...
                     , p.kod_okved as kod_okved
                     , a.ym  as ym
                     , (SELECT (CASE WHEN p_add_rule_opt = 1 AND kod_price_zone in (63, 64) AND (kod_gtp_gp IS NOT NULL AND kod_gtp_gp::text <> '')
                            THEN 1 ELSE pr_opt
                            END) pr_opt
               ...
               Where ((cnt_dep = 0) Or (cnt_dep > 0 And dp.dep in (Select val From vr_number_array Where array_id = 'p_dep')))
                   ...
                   AND (a.price IS NOT NULL AND a.price::text <> '')
                   AND coalesce(a.rym::text, '') = ''
...
;
   ...
   END;  -- end procedure po_per_region_add_regl
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE ng_rep_other_po_per_region_add_regl (p_ym_beg bigint, p_ym_end bigint, p_kod_typevariant bigint default 1, p_add_rule_opt bigint default 0) FROM PUBLIC;



CREATE OR REPLACE PROCEDURE ng_rep_other_po_per_region_sost (p_ym_beg bigint, p_ym_end bigint, is_clear bigint DEFAULT 1, p_vid_calc bigint DEFAULT 0, is_dog bigint DEFAULT 0) AS $body$
DECLARE

        cnt_dep     bigint;   -- кол-во выбранных отделений
        cnt_kod_adr_m bigint;  -- кол-во выбранных субъектов РФ
        cnt_kod_tipdog      bigint;  -- кол-во видов договора
BEGIN
      IF is_clear=1 THEN  DELETE FROM rr_rep_po;END IF;
      ...
      , pre  AS (
                    SELECT a.kod_account
                          , nk_adress.kf_get_sf(CASE WHEN coalesce(ob.kodd::text, '') = '' OR ob.kodd = -1 THEN d.kod_d_dog ELSE ob.kodd END) as kod_region
                          , nk_adress.kp_get_region_name(CASE WHEN coalesce(ob.kodd::text, '') = '' OR ob.kodd = -1 THEN d.kod_d_dog ELSE ob.kodd END) as region_name
                          , d.kod_dog
                          , d.ndog as ndog
                          , a.kod_numobj
                          , d.kodp
   ...
   , itog as (
          SELECT  pre.kod_region as kod_region
              ...
              , CASE WHEN coalesce(a.tarif,0)=0 THEN  0  ELSE kg_tarif.f_byt_cached(a.tarif) END  as pr_byt
          ...
          ,   (CASE WHEN n.edizm =23
              ...
              , (CASE WHEN coalesce(vol.voltage::text, '') = '' THEN -9 ELSE vol.voltage END) as voltage
              , coalesce(vol.abbr,'-')  as volt_abbr
              , (CASE WHEN n.edizm = 23 or (fr.gr_cust = 0 and n.edizm = 3) THEN 0 ELSE coalesce(sost.cust, a.cust) END) as cust   -- натуральные показатели (fr.gr_cust = 0 - в СФ строки нет)
              ...
              , (CASE WHEN p_vid_calc in (0,3) THEN (CASE WHEN coalesce(sost.nal::text, '') = ''     THEN a.nal     ELSE a.nal - sost.nal         END) ELSE coalesce(sost.nal, a.nal) END) as nal
              , (CASE WHEN p_vid_calc in (0,3) THEN (CASE WHEN coalesce(sost.nachisl::text, '') = '' THEN a.nachisl ELSE a.nachisl - sost.nachisl END) ELSE coalesce(sost.nachisl, a.nachisl) END) as nachisl
              ...
              , (CASE WHEN pre.pmax < 670 THEN 'до 670 кВт'   -- 'менее 670 кВт'
                      ...
                      ELSE 'не менее 10000 кВт'
                   END) as max_power
              ,  CASE WHEN fvt.size_doc=0 THEN  fr.group_small                                     WHEN fvt.size_doc=1 THEN  fr.group_big                                     WHEN fvt.size_doc=2 THEN  fr.group_min                                      ELSE fr.group_small_rym END  as gr_str_sf
              , a.pr_hand
          FROM  pre
             ...
             LEFT JOIN( select  nal.kod_account as kod_account
                                ...
                                , round((sum(nal.nal))::numeric,2) as nal
                                , round((sum(nal.nachisl))::numeric,2) as nachisl
             ...
             LEFT JOIN fin_doc.sr_facras_text frt ON a.kod_account = frt.kod_account
             LEFT JOIN fin_doc.sr_facras fr ON frt.kod_ras = fr.kod_ras
             LEFT join fin_doc.sr_facvip fv ON fr.kod_sf = fv.kod_sf and a.kod_dog = fv.kod_dog and a.ym = fv.ym
             LEFT JOIN fin_doc.sr_facvip_text fvt ON fv.kod_sf = fvt.kod_sf
             LEFT JOIN ec_account.hk_voltage vol ON a.voltage = vol.voltage
             LEFT JOIN payer.kr_payer p ON pre.kodp = p.kodp
             LEFT JOIN fin_doc.sk_nachisl n ON a.vid_t = n.vid_t
             Cross Join pars
         WHERE   n.vid_real = 2
            AND ((p_vid_calc in (0,3) AND coalesce(a.rym::text, '') = '' )  -- факт или факт предыдущего месяца
              OR (p_vid_calc = 1 AND (a.rym IS NOT NULL AND a.rym::text <> '')) --a.rym = kg.ym_add(a.ym,-1))      -- корректировки
              OR (p_vid_calc = 2 AND (sost.kod_account IS NOT NULL AND sost.kod_account::text <> '')))
...
;
   END;  -- end procedure po_per_region_sost
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE ng_rep_other_po_per_region_sost (p_ym_beg bigint, p_ym_end bigint, is_clear bigint DEFAULT 1, p_vid_calc bigint DEFAULT 0, is_dog bigint DEFAULT 0) FROM PUBLIC;



CREATE OR REPLACE PROCEDURE ng_rep_other_payer_employee () AS $body$
BEGIN
   DELETE FROM rr_rep_emp;
 ...
 SELECT  p.kodp as payer_id
       ...
       , ng_rep_other_get_phone_all(p.kodp) as tel_all
       , ng_rep_other_get_email_all(p.kodp) as email_all
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE ng_rep_other_payer_employee () FROM PUBLIC;
...
CREATE OR REPLACE PROCEDURE ng_rep_other_po_on_retail (p_ym_beg bigint, p_ym_end bigint, p_dep bigint) AS $body$
BEGIN
    DELETE FROM rr_rep_po;
    ...
    SELECT ( nk_adress.kf_get_sf(d.kod_d_dog)) as kod_region   -- для розницы регион берем из договора
           ...
           , round((sum(CASE WHEN nn.edizm in (4,23) THEN fr.nachisl - COALESCE(nal.nal,0) ELSE 0 END))::numeric,2) as nach
           , round((sum(CASE WHEN nn.edizm in (3)    THEN fr.nachisl - COALESCE(nal.nal,0) ELSE 0 END))::numeric,2) as nach_kvt
     ...
     FROM dog.kr_dogovor ds
         ...
         INNER JOIN fin_doc.sk_nachisl nn ON fr.vid_t = nn.vid_t
           LEFT JOIN(SELECT SUM(coalesce(nal,0)) as nal, kod_ras FROM fin_doc.sr_facras_nal GROUP BY kod_ras) nal ON fr.kod_ras = nal.kod_ras
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE ng_rep_other_po_on_retail (p_ym_beg bigint, p_ym_end bigint, p_dep bigint) FROM PUBLIC;
-- End of Oracle package 'ng_rep_other' declaration
