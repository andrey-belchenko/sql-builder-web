...
//Must be refactored with webRequest (for net4.0)
			...
			using (System.Net.Http.HttpClient client = new System.Net.Http.HttpClient())
			{
				...
				if (!result.Result.IsSuccessStatusCode)
				{
					...
					var projectListObject = JsonConvert.DeserializeObject(result.Result.Content.ReadAsStringAsync().Result);
		...
		internal static string[] GetDataExtractNameListByProject(string project)
		{
			...
			using (System.Net.Http.HttpClient client = new System.Net.Http.HttpClient())
			{
				...
				{
					var extractListObject = JsonConvert.DeserializeObject(result.Result.Content.ReadAsStringAsync().Result);
