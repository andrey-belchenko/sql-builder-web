...
namespace sql.builder
{
    public static class Program
    {
        ...
        {
            var tableNames = DevSqlParserAntlr.GetSourceTables(@"CREATE OR REPLACE PACKAGE BODY kg_rep_dog
  ...
  -- p_ym - фильтр для вывода только активных договоров в заданный период
    ...
    BEGIN
      ...
      INSERT INTO rr_rep_dog ( kod_dog
                            ...
                            , kod_perper      -- периодичность перерасчетов: 0 -
                            )
      SELECT d.kod_dog
          , d.ndog
          , p.kodp as payer_id
   ...
   BEGIN
    ...
    THEN
       ...
       SELECT 'p_kod_dog'
              ,kod_dog
        FROM per
       WHERE kod_perper = 0;   -- периодичность перерасчетов: perper = 0
    ELSE
