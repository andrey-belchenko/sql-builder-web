    ...
    internal class ExcelPrintEnv : IDisposable
    {
        ...
        internal ExcelPrintEnv(string template_path, ExcelPrintOptions options, DataSet data)
        {
            ...
            using (ZipArchive xlsx = ZipFile.OpenRead(template_path)) {
                xlsx.ExtractToDirectory(_print_directory, true);
            }
        ...
        internal void Save(string output_path)
        {
            ...
            using (ZipArchive zip = ZipFile.Open(output_path, ZipArchiveMode.Create)) {
                foreach (string filePath in Directory.GetFiles(_print_directory, "*", SearchOption.AllDirectories)) {
                    string relativePath = filePath.Substring(_print_directory.Length).TrimStart(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
