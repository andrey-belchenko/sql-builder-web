    ...
    internal static class SqlRepository
    {
      ...
      //  private static string key_changed_name;
        ...
        public static string GetPackageSql()
        {
         ...
         //   sql.AppendLine("CREATE OR REPLACE PACKAGE VG_DWH_UPD_" + rep_name + " AUTHID CURRENT_USER");
         //   sql.AppendLine("IS");
         //   sql.AppendLine("\ts_tmp varchar2(200);");
         //   sql.AppendLine("\tn_tmp number;");
         //   sql.AppendLine();
         //   sql.AppendLine("\tPROCEDURE MAIN;");
         // //  sql.AppendLine("\tPROCEDURE " + rep_name + "_UPD;");
         //   sql.AppendLine("END;");
         //   sql.AppendLine();
         //   sql.AppendLine("CREATE OR REPLACE PACKAGE BODY VG_DWH_UPD_" + rep_name + "");
         //   sql.AppendLine("IS");
         ...
         //   sql.AppendLine("\t\t--Получаем список измененных кодов");
         //   sql.AppendLine("\t\t--Если хранилище ни разу не формировалось полностью - список измененных кодов так же будет пустым");
         //  // sql.AppendLine("\t\tselect substr(stragg_dist(" + key_changed_name + "), 1, 470) into s_info from (" + query_changed_sql + ");");
         //   sql.AppendLine("\t\t--Если кодов нет, значит и обновлять нечего");
         //   sql.AppendLine("\t\tif(s_info is null) then");
         ...
         //   sql.AppendLine("\t\t--Удаление старых данных по условию");
         ////   sql.AppendLine(String.Format("\t\tdelete {0} where {1} in ({2});", rep_name, key_changed_name, query_changed_sql));

         //   sql.AppendLine("\t\t--Обновление данных");

         ////   AddLogSql("Начало обновления", "'"+ key_changed_name + " in '||s_info", 2);

         //   sql.AppendLine("\t\td_start := sysdate;");
         //   sql.AppendLine("\t\tinsert into " + rep_name);
         //   sql.AppendLine(query_upd_sql);
         //   sql.AppendLine("\t\t;");

         //   AddRepUpdInfoSql("d_start", 2);
         //   AddLogSql("Успешно обновлено", "''", 2);
         //   AddUnlockRepositorySql(2);
         //   AddLogSql("Блокировка снята", "''", 2);
         ...
         //   AddLogSql("Ошибка обновления", "s_result", 4);
         //   AddUnlockRepositorySql(4);
         //   AddLogSql("Блокировка снята", "''", 4);
        ...
        //public static string GetDimTableSql()
        //{
        //    Initialize();
        ...
        //    sql += "drop table " + dim_tab_name + ";"+nl;

        //    sql += "create table " + dim_tab_name + "(";
        //    string q="";
        //    foreach (XElement col in changes_scheme.Descendants("table").First().Element("columns").Elements())
        //    {
        //        if (col.Attribute("name").Value != dateChangeName)
        //        {
        //            string typ=col.Attribute("type").Value;
        //            if (typ == "string")
        //            {
        //                typ = "varchar(1000)";
        //            }
        //            sql+=col.Attribute("name").Value+" "+typ+q;
        //            q = ", ";
        //        }
        //    }
        //    sql += ");" + nl;

        //    sql += "grant select , update, insert, delete on " + dim_tab_name + " to public;" + nl;
        //    GetChangeJoinElement();
        //    return sql;
        //}
